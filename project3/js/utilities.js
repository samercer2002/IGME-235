  ...
  function getPlayerDirection(x, y, player)
  {
    ...
    let yComp = player.y - y;
    if(Math.abs(xComp) > Math.abs(yComp) && xComp > 0)
    {
        return {x:1, y:0}
    }
    else if (Math.abs(xComp) > Math.abs(yComp) && xComp < 0)
    {
        return {x:-1, y:0}
    }
    else if(Math.abs(xComp) < Math.abs(yComp) && yComp > 0)
    {
        return {x:0, y:1}
    }
    else if(Math.abs(xComp) < Math.abs(yComp) && yComp < 0)
